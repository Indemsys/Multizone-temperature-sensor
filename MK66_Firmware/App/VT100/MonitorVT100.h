...
#define GET_MCBL     T_monitor_cbl  *mcbl =  (T_monitor_cbl *)_task_get_environment(_task_get_id())
#define MPRINTF      mcbl->_printf
#define WAIT_CHAR    mcbl->_wait_char
...
typedef struct
{
    ...
    int                   (*_wait_char)(unsigned char *b,  int ticks);      // ticks - время ожидания в тиках (если 0, то без ожидания)
...
int32_t      Prepare_monitor_IO_driver(void);
...
_mqx_uint    Edit_integer_hex_val(uint32_t row, uint32_t *value, uint32_t minv, uint32_t maxv, const char *fmt);
