...
uint32_t      rtu_timer_tick_freq;             // Частота тиков таймера таймаута RTU
...
#if (MODBUS_CFG_RTU_EN == 1)
    pch->rtu_timeout_en = 1;
#endif

#if (MODBUS_CFG_SLAVE_EN == 1)  && (MODBUS_FC08_EN      == 1)
    MB_S_StatInit(pch);
#endif
    pch++;
  }
                      ...
                      uint8_t  stops)
{
  ...
  if (modbus_cbls_counter < MODBUS_CFG_MAX_CH)
  {
    ...
    {
      pch->rtu_timeout_en = 0;
    }
    //cnts =((uint32_t)rtu_timer_tick_freq * 5L * 10L) / baud;     // Freq * 5 char * 10 bits/char * 1/BaudRate
...
void  MB_slave_rx_task(T_MODBUS_ch *pch)
{
  ...
  if (pch != (T_MODBUS_ch *)0)
  {
    if (pch->master_slave_flag == MODBUS_SLAVE)
    {
      MB_S_rx_task(pch);
    }
  }
