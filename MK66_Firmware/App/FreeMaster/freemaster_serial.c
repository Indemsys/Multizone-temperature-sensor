...
static void FMSTR_Process_OS_IO(void)
{
  ...
  if (!fms_flags.flg.tx_active)
  {
    uint8_t rx_char;
    do
    {
      if (frm_serial_drv->_wait_char(&rx_char, 2) == MQX_OK) if (FMSTR_Rx(rx_char)) break;
    }
    while (1);
  }
  ...
  if (fms_flags.flg.tx_active)
  {
    ...
    while (1)
    {
      ...
      {
        // Данных больше нет отправляем то, что накопили в промежуточный буффер
        if (cnt > 0) frm_serial_drv->_send_buf(out_buff, cnt);
        break;
      }
