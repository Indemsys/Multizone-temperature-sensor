  ...
  case USB_DEV_EVENT_DATA_RECEIVED:
    {
      ...
      if (vcom1_cbl.configured == TRUE)
      {
        ...
        if ((sz != 0xFFFFFFFF) && (sz != 0))
        {
          ...
          vcom1_cbl.rx_pack_cbl[vcom1_cbl.rx_head].len =sz;
          ...
          else
          {
            ...
            vcom1_cbl.rx_full = 1;
            vcom1_cbl.rx_errors1++;

          }
          _lwevent_set(&vcom1_cbl.os_flags, VCOM_FLAG_RECEIVED); // Установливаем флаг получения данных
        }
        else
        {
...
uint32_t VCom1_USB_send_data(uint8_t *buff, uint32_t size, uint32_t ticks)
{
  ...
  if (vcom1_cbl.tx_enabled != TRUE)
  {
    vcom1_cbl.tx_errors1++;
    return MQX_ERROR;
  }
...
uint32_t VCom1_USB_get_data(uint8_t *b, uint32_t ticks)
{
  ...
  if (vcom1_cbl.rx_tail == vcom1_cbl.rx_head)
  {
    if (ticks != 0)
    {
      if (_lwevent_wait_ticks(&vcom1_cbl.os_flags, VCOM_FLAG_RECEIVED, FALSE, ticks) != MQX_OK)
      {
        return MQX_ERROR;
      }
  ...
  buf_n = vcom1_cbl.rx_tail;
  *b = vcom1_cbl.rx_pack_cbl[buf_n].buff[vcom1_cbl.rx_pack_cbl[buf_n].pos];
  vcom1_cbl.rx_pack_cbl[buf_n].pos++;
  pos = vcom1_cbl.rx_pack_cbl[buf_n].pos;

  // Если позиция достигла конца данных в текущем буфере, то буфер освобождается для приема
  if (pos >= vcom1_cbl.rx_pack_cbl[buf_n].len)
  {
