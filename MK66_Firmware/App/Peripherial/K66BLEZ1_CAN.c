...
-------------------------------------------------------------------------------------------------------------*/
static _mqx_uint Create_CANTX_control_sync_obj(void)
{
  _mqx_uint res;
  ...
  can_tx_queue_id = _msgq_open(CAN_TX_QUEUE, CAT_TX_QUEUE_SZ);
...
# endif
  ...
  // Настройка вручную скорости CAN на 500000 бит/сек. Частота тактирования CAN периферии здесь равна 60 МГц
  ...
  CAN_PTR->CTRL1 = 0
                  ...
                  + LSHIFT(0, 15) // BOFFMSK. Bus Off Mask. 1 Bus Off interrupt enabled
                  + LSHIFT(0, 14) // ERRMSK . Error Mask.   1 Error interrupt enabled
...
_mqx_uint CAN_post_packet_to_send(uint32_t canid, uint8_t *data, uint8_t len)
{
  ...
  if (cmpt != NULL)
  {
    ...
    cmpt->header.SOURCE_QID = cmpt->header.TARGET_QID;
