...
static T_qdec_isr  qdec1_isr;
static T_qdec_isr  qdec2_isr;
...
static void FTM1_isr(void *user_isr_ptr)
{
  ...
  if (sc & BIT(7))
  {
    ...
    if (qdec1_isr != 0) qdec1_isr(FTM1->QDCTRL & BIT(1));
  }
}

/*-----------------------------------------------------------------------------------------------------
  Прерывание от таймера FTM2 обслуживающее счетчик квадратурного энкодера
...
static void FTM2_isr(void)
{
  ...
  sc =  FTM2->SC; // Читаем состояние
  if (sc & BIT(7))
  {
    FTM2->SC = sc & ~(BIT(7)); // Сбрасываем бит TOF

    if (qdec2_isr != 0) qdec2_isr(FTM2->QDCTRL & BIT(1));
  }
}
...
/*-----------------------------------------------------------------------------------------------------
  Конвертер коэффициента предделителя в значение поля регитсра

  \param presc  - значение предделителя. Может принимать только определенные значения

  \return uint8_t
-----------------------------------------------------------------------------------------------------*/
uint8_t _Presc_to_register_field(uint8_t presc)
{
  switch (presc)
  {
  case 1:
...
/*-----------------------------------------------------------------------------------------------------
 Включить тактирование на заданном FTM

 \param FTM
-----------------------------------------------------------------------------------------------------*/
static void FTM_switch_on_clk(FTM_MemMapPtr FTM)
{
...
void FTM1_init_QDEC(T_qdec_isr isr)
{
  ...
  // Разрешаем тактирование FTM1
  FTM_switch_on_clk(FTM1_BASE_PTR);
...
/*------------------------------------------------------------------------------
  Конфигурирование FTM2 в режим квадратурного декодера
...
//  SIM_SCGC3 |= BIT(24);  // Разрешаем тактирование FTM2 на AIPS1 (Peripheral bridge 1)
  ...
  FTM2->SC = 0
            ...
            + LSHIFT(0, 5) // CPWMS | 1 FTM counter operates in up-down counting mode.
            + LSHIFT(0, 3) // CLKS  | 00 Модуль таймера остановлен
            + LSHIFT(0, 0) // PS    | Prescale Factor Selection. 000 Divide by 1. Предделитель = 1, 100 Divide by 16
  ;
  FTM2->COMBINE  = 0;     // Связанную работу каналов не используем

  FTM2->MOD      = 0xFFFF;    // Установка регистра перезагрузки.
  FTM2->CNTIN    = 1;         // Начальное значение счетчика. Устанавливаем 1 чтобы первый же импульс не вызвал прерывания по переполнению
  ...
  FTM2->QDCTRL  = 0
                 ...
                 + LSHIFT(0, 2) // QUADIR (ro)| FTM Counter Direction In Quadrature Decoder Mode
                 + LSHIFT(0, 1) // TOFDIR (ro)| Timer Overflow Direction In Quadrature Decoder Mode
                 + LSHIFT(1, 0) // QUADEN     | Quadrature Decoder Mode Enable
  ;
  ...
  FTM2->SC = 0
            ...
            + LSHIFT(0, 5) // CPWMS. 1 -FTM counter operates in up-down counting mode.
            + LSHIFT(3, 3) // CLKS. 11 External clock
            + LSHIFT(0, 0) // PS. Prescale Factor Selection. Prescale Factor Selection. 000 Divide by 1. Предделитель = 1, 100 Divide by 16
  ;



  qdec2_isr = isr;
  Install_and_enable_kernel_isr(INT_FTM2, FTM2_KERN_PRIO, FTM2_isr); // Разрешение прерывания по переполнению счетчика
}
...
void FTM_init_WS2812_PWM_DMA(FTM_MemMapPtr FTM)
{
  ...
  FTM_switch_on_clk(FTM);

  st = FTM->SC; // Это нужно сделать чтобы был стерт флаг прерывания
  FTM->SC = 0; // Останавливаем таймер
...
/*-----------------------------------------------------------------------------------------------------
 Инициализация FTM для генерации 3-х фазного PWM сигнала управления мотором
 Применяется центрально-симметричная модуляция
 Прерывания запрещены. Предполагается их разрешение при страте таймера

  Назначение каналов таймера:
 0 -  верхний ключ канала A
 1 -  нижний  ключ канала A
 2 -  верхний ключ канала B
 3 -  нижний  ключ канала B
 4 -  верхний ключ канала C
 5 -  нижний  ключ канала C
 6 -  не используется
 7 -  не используется
...
void FTM_init_BLDC_PWM(FTM_MemMapPtr FTM, uint32_t mod)
{
  ...
  FTM->MOD     = mod;  // Установка регистра перезагрузки.
  FTM->CNTIN   = 0;    // Начальное значение счетчика
  FTM->CNT     = 0;    // Запись в регистр счетчка любого значения приводит к записи значения из CNTIN и установке начального состояния выходов
  FTM->POL     = 0;    // Полярность у сигналов
  ...
  // Устанавливаем какие внешние тригеры будут активными. Триггеры используются другими модулями. Модуль ADC использует триггеры для старта преобразования
  FTM->EXTTRIG = 0
                + LSHIFT(0, 7)  // TRIGF      | Channel Trigger Flag
                + LSHIFT(1, 6)  // INITTRIGEN | Initialization Trigger Enable | Enables the generation of a trigger when the FTM counter is equal to its initial value
  ...
  // В послудующих процедурах следует:
  // Install_and_enable_kernel_isr - установить вектор прерываний если требуется
  // FTM_start                     - запустить таймер
  //
}

/*-----------------------------------------------------------------------------------------------------
 Инициализация FTM для PWM сигналов
 Применяется  модуляция по срезу (не центарльно симметричная)
 Прерывания запрещены. Предполагается их разрешение при страте таймера
...
void FTM_init_PWM(FTM_MemMapPtr FTM, uint32_t mod)
{
  ...
  FTM->OUTMASK = 0    // Маскировка реакции выходов на событие совпадения, т.е. маска отключения PWM на заданных выходах
                + LSHIFT(0, 7)  // CH7OM | 1 Channel output is masked. It is forced to its inactive state.
                + LSHIFT(0, 6)  // CH6OM | 1 Channel output is masked. It is forced to its inactive state.
                + LSHIFT(0, 5)  // CH5OM | 1 Channel output is masked. It is forced to its inactive state.
                + LSHIFT(0, 4)  // CH4OM | 1 Channel output is masked. It is forced to its inactive state.
                + LSHIFT(0, 3)  // CH3OM | 1 Channel output is masked. It is forced to its inactive state.
                + LSHIFT(0, 2)  // CH2OM | 1 Channel output is masked. It is forced to its inactive state.
                + LSHIFT(0, 1)  // CH1OM | 1 Channel output is masked. It is forced to its inactive state.
                + LSHIFT(0, 0)  // CH0OM | 1 Channel output is masked. It is forced to its inactive state.
  ;
  FTM->COMBINE = 0     // Связанную работу каналов не используем
                       // Установки для  каналов 6 и 7
                + LSHIFT(0, 30) // FAULTEN3. 1 The fault control in this pair of channels is enabled.
  ...
  FTM->DEADTIME = 0 // Мертове время
                 ...
                 + LSHIFT(15,  0) // DTVAL | Deadtime Value           | 0.25 мкс
  ;
  FTM->INVCTRL = 0;  // Управление взаимным обменом выходов пар каналов
  FTM->FILTER = 0;   // Фильтры для входных каналов не используем
  ...
  // В последующих процедурах следует:
  // Install_and_enable_kernel_isr - установить вектор прерываний если требуется
  // FTM_start                     - запустить таймер
  //
}
...
void FTM1_FTM2_init_hall_sens_meas(uint8_t presc, uint32_t mod)
{
  ...
  FTM2->SC = 0
            ...
            + LSHIFT(0, 5) // CPWMS | 1 FTM counter operates in up-down counting mode.
            + LSHIFT(0, 3) // CLKS  | 00 Модуль таймера остановлен
            + LSHIFT(presc, 0) // PS    | Prescale Factor Selection. 000 Divide by 1. Предделитель = 1, 100 Divide by 16
  ;
  FTM2->COMBINE = 0;     // Связанную работу каналов не используем
  ...
  FTM2->MOD      = mod;    // Установка регистра перезагрузки.
  FTM2->CNTIN    = 0;         // Начальное значение счетчика
  FTM2->CNT      = 0;         // Запись в регистр счетчка любого значения приводит к записи значения из CNTIN и установке начального состояния выходов

  FTM1->MOD      = mod;    // Установка регистра перезагрузки.
  FTM1->CNTIN    = 0;         // Начальное значение счетчика
  FTM1->CNT      = 0;         // Запись в регистр счетчка любого значения приводит к записи значения из CNTIN и установке начального состояния выходов
  ...
  FTM2->SC = 0
            ...
            + LSHIFT(presc, 0) // PS. Prescale Factor Selection. Prescale Factor Selection. 000 Divide by 1. Предделитель = 1, 100 Divide by 16
  ;
  // Запускаем системный тактирующий сигнал на таймер
  FTM1->SC = 0
            + LSHIFT(0, 6) // TOIE. 1 Enable TOF interrupts. An interrupt is generated when TOF equals one.
            ...
            + LSHIFT(presc, 0) // PS. Prescale Factor Selection. Prescale Factor Selection. 000 Divide by 1. Предделитель = 1, 100 Divide by 16
  ;
  DELAY_1us; // Задержка для того чтобы успело сработать прерывание
  FTM2->CONTROLS[1].CnSC = FTM2->CONTROLS[1].CnSC & ~(BIT(7));  // Сброс прерывания возникшего в момент инициализации
...
/*-----------------------------------------------------------------------------------------------------
  Устанавливаем модуль и предделитель таймера
...
{
  FTM->SC = FTM->SC & ~(LSHIFT(3, 3));
  // Записываем значение для предделителя
  FTM->SC =(FTM->SC & ~(LSHIFT(7, 0))) | _Presc_to_register_field(presc);
  FTM->MOD     = mod;    // Установка регистра перезагрузки.
}
...
{
  FTM->CONTROLS[0].CnV = val;
  FTM->CONTROLS[1].CnV = val;
  FTM->CONTROLS[2].CnV = val;
  FTM->CONTROLS[3].CnV = val;
  FTM->CONTROLS[4].CnV = val;
  FTM->CONTROLS[5].CnV = val;
}
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param FTM
  \param ch   - канал таймера
  \param proc - скважность ШИМ в процентах
-----------------------------------------------------------------------------------------------------*/
void FTM_set_PWM(FTM_MemMapPtr FTM, uint8_t ch, uint32_t percent)
{
  if (ch >= 8) return;
  FTM->CONTROLS[ch].CnV =(FTM->MOD *  percent) / 100;
  FTM->SYNC |= BIT(7); // Запускаем процесс синхронной загрузки регистров
}


/*-----------------------------------------------------------------------------------------------------
  Запуск работы таймера и одновременно генерации сигналов на выходах

  \param FTM
-----------------------------------------------------------------------------------------------------*/
void FTM_start(FTM_MemMapPtr FTM, uint8_t en_int)
{
  uint32_t     sc_reg = FTM->SC;

  FTM->CNT     = 0;
  if (en_int == 0)
  {
    FTM->SC =(sc_reg & ~((LSHIFT(3, 3))+ BIT(6))) | LSHIFT(1, 3); // CLKS  | 01 System clock;
  }
  else
  {
    FTM->SC =(sc_reg & ~(LSHIFT(3, 3))) | LSHIFT(1, 3) | BIT(6); // CLKS  | 01 System clock;
  }
}

/*-----------------------------------------------------------------------------------------------------
  Остановка таймера и одновременно генерации сигналов на выходах

  \param FTM
-----------------------------------------------------------------------------------------------------*/
void FTM_stop(FTM_MemMapPtr FTM)
{
  FTM->SC = FTM->SC & ~(LSHIFT(3, 3)); // Устанавливаем CLKS=0, запрещаем работу таймера
  FTM->OUTINIT = 0;    // Переводим все выходы в 0
  FTM->OUTMASK = 0;    //
  FTM->SWOCTRL = 0;    //
}
