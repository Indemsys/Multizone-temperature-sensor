...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param baud
-----------------------------------------------------------------------------------------------------*/
void SPI0_set_CTAR(T_SPI_bits fsz, uint8_t cpol, uint8_t cpha, T_SPI_bauds baud)
{
  uint8_t       dbr    = 1;
  uint8_t       pbr    = 3;
  uint8_t       br     = 2;
...
// Задержка задержка подачи тактового сигнала после установки сигнала выборки должна быть равна половине длительности тактового сигнала
  ...
  {
  case SPI_BAUD_30MHZ   :
    pbr = PBR_2; br = BR_2; pcssck = PCSSCK_1; cssck = CSSCK_2;  break;   //
  case SPI_BAUD_20MHZ   :
  ...
  case SPI_BAUD_750KHZ :
    pbr = PBR_5; br = BR_32; pcssck = PCSSCK_5; cssck = CSSCK_4;  break;   //
  }
  ...
  SPI->CTAR[0] = 0
    ...
    + LSHIFT(1,       8)  // ASC    | After SCK Delay Scaler. The After SCK Delay is the delay between the last edge of SCK and the negation of PCS
...
/*-------------------------------------------------------------------------------------------------------------
  ...
  mtfe  - флаг модифицированного тайминга
...
-------------------------------------------------------------------------------------------------------------*/
void SPI_master_init(uint8_t modn, T_SPI_bits fsz, uint8_t cpol, uint8_t cpha, T_SPI_bauds baud, uint8_t mtfe, uint8_t enint)
{
  ...
  SPI->MCR = 0
    ...
    + LSHIFT(mtfe, 26) // MTFE      | Modified Timing Format Enable                | 0 Modified SPI transfer format disabled.
...
void SPI_clear_FIFO(uint8_t modn)
{
  ...
  SPI->MCR |= BIT(0); // Остановить модуль
  while (SPI->SR & BIT(30)); // Ожидание пока бит 30 (TXRXS) не станет равным нулю

  SPI->SR = SPI->SR;
  SPI->MCR |= BIT(11) + BIT(12);
  SPI->MCR &= ~BIT(0); // Запустить модуль

}

/*-----------------------------------------------------------------------------------------------------

 \param modn   - номер записи в массиве конфигурационных параметров
 \param fsz    - Frame Size интерфейса
 \param cpol   - параметр cpol интерфейса
 \param cpha   - параметр cpha интерфейса
 \param baud   - скрость интерфейса
 \param star_n - номер конфигурационного регистра, есть два конфигурационных регистра для обслуживания обмена с двумя разными типами периферии
-----------------------------------------------------------------------------------------------------*/
void SPI_set_star(uint8_t modn, T_SPI_bits fsz, uint8_t cpol, uint8_t cpha, T_SPI_bauds baud, uint8_t star_n)
{
  uint8_t       dbr    = 1;
  uint8_t       pbr    = 3;
  uint8_t       br     = 2;
  ...
  // Запрещаем прерываний
  _int_disable();

  SPI->MCR |= BIT(HALT); // Остановить
                         // Ждем пока SPI перейдет в режим STOP
  while (SPI->SR & BIT(30))
  {
    __no_operation();
  }
...
// Задержка задержка подачи тактового сигнала после установки сигнала выборки должна быть равна половине длительности тактового сигнала
  ...
  SPI->CTAR[star_n] = 0
    ...
    + LSHIFT(0,       24) // LSBFE  | LSB First                              | 0 Data is transferred MSB first.
    + LSHIFT(pcssck,  22) // PCSSCK | PCS to SCK Delay Prescaler             | 01 PCS to SCK Prescaler value is 3.
    + LSHIFT(1,       20) // PASC   | After SCK Delay Prescaler              | 00 Delay after Transfer Prescaler value is 1.
    + LSHIFT(0,       18) // PDT    | Delay after Transfer Prescaler         | 00 Delay after Transfer Prescaler value is 1.
    + LSHIFT(pbr,     16) // PBR    | Baud Rate Prescaler. The baud rate is the frequency of the SCK (0 -> 2, 1 -> 3, 2 -> 5, 3 -> 7)
    + LSHIFT(cssck,   12) // CSSCK  | PCS to SCK Delay Scaler. The PCS to SCK Delay is the delay between the assertion of PCS and the first edge of the SCK
    + LSHIFT(1,       8)  // ASC    | After SCK Delay Scaler. The After SCK Delay is the delay between the last edge of SCK and the negation of PCS
