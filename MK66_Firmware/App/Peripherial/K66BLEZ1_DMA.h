...
typedef struct
{
  ...
  uint8_t             enable_interrupt; // Флаг разрешения прерывания после мажорного циккла DMA
} T_DMA_config;


typedef struct
{
  FTM_MemMapPtr       FTM;        // Указатель на таймер
  ...
  DMAMUX_MemMapPtr    DMAMUX;     // Указатель на структуру мультиплексора
...
typedef struct
{
  ...
  DMAMUX_MemMapPtr    DMAMUX;
  uint32_t            dmux_src;   // номер входа периферии для выбранного мультиплексора DMAMUX для передачи на DMA. Например DMUX0_SRC_SPI0_RX
  uint8_t             minor_tranf_unit_sz; // Размер в байтах слова передаваемого по DMA в минорном цикле
  uint8_t             minor_tranf_sz;      // Количество пересылаемых байт в минорном цикле
...
#define  SPI0_BUS_RXEND   BIT(1)  // Флаг передаваемый из ISR об окончании приема по SPI


extern T_DMA_cbl SPI0_DS_cbl;
