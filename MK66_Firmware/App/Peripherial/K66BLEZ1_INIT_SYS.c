...
//  Инициализация чипа
// 
//  Частота ядра = 180 МГц
//  Частота внутренней шины и таймеров = 60 МГц 
//  flash clock = 25.7 MHz 
//
//-------------------------------------------------------------------------------------------------------
void Init_cpu(void)
{
...
{
  NVIC_MemMapPtr NVIC = NVIC_BASE_PTR;
  if  (irq_index < 16) return; // Системные исключения нельзя не разрешить, не запретить
...
void NVIC_SetPriority(int32_t irq_index, uint32_t priority)
{
  ...
  if (IRQn < 0)
  {
    uint8_t n = ((uint32_t)(IRQn)&0xF) - 4;
    uint32_t pri = ((priority << (8 - __NVIC_PRIO_BITS)) & 0xff);
    switch (n)
    {
    case 0:  SCB->SHPR1 = pri; break;
    case 1:  SCB->SHPR2 = pri; break;
    case 2:  SCB->SHPR3 = pri; break;
    }  
  }  
  else
  {
