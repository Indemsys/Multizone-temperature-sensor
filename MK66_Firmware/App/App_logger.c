...
_mqx_uint AppLogg_init(void)
{
  ...
  if (_lwsem_create(&log_entry_sem, 0) != MQX_OK) res = MQX_ERROR;
...
#endif
    //PCLnk_set_log_flag();
    _lwsem_post(&log_entry_sem); // Регистрируем семафор новой записи
    _lwsem_post(&log_sem);       // Освобождаем семафор доступа
  }
  else
  {
    log_cbl.sync_err++;
...
/*------------------------------------------------------------------------------
 ...
 \param pvt100_cb
 ------------------------------------------------------------------------------*/
void AppLogg_monitor_output_prev(T_monitor_cbl *mcbl)
{
  uint32_t   i;
  uint32_t   n;
  uint32_t   reqn;
  ...
  do
  {
    ...
    for (i = 0; i < EVENT_LOG_DISPLAY_ROW; i++)
    {
      if (i < outn)
      {
        ...
        {
          MPRINTF(VT100_CLL_FM_CRSR"%05d.%03d %s (%s %d)\n\r",
        ...
        {
          MPRINTF(VT100_CLL_FM_CRSR"%05d.%03d %s\n\r",
                  app_log[n].time.SECONDS, app_log[n].time.MILLISECONDS,
                  app_log[n].msg);
        }
        n++;
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param mcbl
-----------------------------------------------------------------------------------------------------*/
void AppLogg_monitor_output(T_monitor_cbl *mcbl)
{
  uint32_t   idx;
  uint8_t    b;
  ...
  do
  {
    ...
    while (idx != log_cbl.event_log_head)
    {
      ...
      {
        MPRINTF(VT100_CLL_FM_CRSR"%05d.%03d %s (%s %d)\n\r",
                app_log[idx].time.SECONDS, app_log[idx].time.MILLISECONDS,
                app_log[idx].msg,
...
void Task_file_log(unsigned int initial_data)
{
  ...
  tstr = (char *)_mem_alloc_zero(LOG_STR_MAX_SZ+1);
  if (tstr == NULL)
  {
  ...
  do
  {
    ...
    if (_lwmsgq_receive((void *)file_log_queue, (uint32_t *)&plrec, LWMSGQ_RECEIVE_BLOCK_ON_EMPTY, 2, 0) == MQX_OK)
    {
      ...
      do
      {
        ...
        res = snprintf(tstr, LOG_STR_MAX_SZ, "%04d.%02d.%02d %02d:%02d:%02d.%03d |",date.YEAR, date.MONTH, date.DAY, date.HOUR, date.MINUTE, date.SECOND, plrec->time.MILLISECONDS);
        if (res > 0) _io_write(f, tstr, res);
        res = snprintf(tstr, LOG_STR_MAX_SZ, "%02d | %-36s | %5d |", plrec->severity, plrec->func_name, plrec->line_num);
        if (res > 0) _io_write(f, tstr, res);
        res = snprintf(tstr, LOG_STR_MAX_SZ, " %s\r\n", plrec->msg);
        if (res > 0) _io_write(f, tstr, res);
        _mem_free(plrec);
        ...
        if (flog_cbl.alloc_err != 0)
        {
          ...
          res = snprintf(tstr, LOG_STR_MAX_SZ, "%04d.%02d.%02d %02d:%02d:%02d.%03d |",date.YEAR, date.MONTH, date.DAY, date.HOUR, date.MINUTE, date.SECOND, plrec->time.MILLISECONDS);
          if (res > 0) _io_write(f, tstr, res);
          res = snprintf(tstr, LOG_STR_MAX_SZ, "ERROR: Allocation fault (%d)\r\n", flog_cbl.alloc_err);
          if (res > 0) _io_write(f, tstr, res);
        ...
        if (flog_cbl.queue_err != 0)
        {
          ...
          res = snprintf(tstr, LOG_STR_MAX_SZ, "%04d.%02d.%02d %02d:%02d:%02d.%03d |",date.YEAR, date.MONTH, date.DAY, date.HOUR, date.MINUTE, date.SECOND, plrec->time.MILLISECONDS);
          if (res > 0) _io_write(f, tstr, res);
          res = snprintf(tstr, LOG_STR_MAX_SZ, "ERROR: Messages queue fault (%d)\r\n", flog_cbl.queue_err);
          if (res > 0) _io_write(f, tstr, res);
    ...
    else
    {
      ...
      if (g_fs_free_space_val_ready == 1)
      {
        g_fs_free_space_val_ready = 2;
        LOGs(__FUNCTION__, __LINE__, SEVERITY_RED, "uSD card free space = %lld MiB", g_fs_free_space / (1024 * 1024));
      }
    }
  }while (1);
