...
/*-----------------------------------------------------------------------------------------------------
  Фильтр удаляющий постоянную составляющую. Задерживает до границы 10 Гц при частоте сэмплирования 100 Гц
  ...
  \return int32_t
-----------------------------------------------------------------------------------------------------*/
int32_t DCremover_filter(int32_t inp, T_dc_remover_filter *fltr)
{
  int16 output;
  if (fltr->en == 0)
  {
  ...
  output = inp - (fltr->arr[0] / 8);

  fltr->arr[0] = output + fltr->arr[0];
  return output;
