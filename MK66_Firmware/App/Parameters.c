...
int32_t Restore_parameters_and_settings(void)
{
  ...
  snprintf(fw_version, FW_VERSION_STR_LEN, "%s %s", __DATE__, __TIME__);
  return (MQX_OK);
}
...
void Edit_func(uint8_t b)
{
  ...
  case 0x0D:  // Enter
    ...
    Save_Params_to_file();  // Сохраняем параметры в файл
...
---------------------------------------------------------------------------------------------*/
_mqx_int Restore_from_file(void)
{
  MQX_FILE_PTR       f;
  _mqx_int           res;
  char              *str;
...
/*-------------------------------------------------------------------------------------------
   Процедура сохранения в ini-файл параметров
---------------------------------------------------------------------------------------------*/
_mqx_int Save_Params_to_file(void)
{
  MQX_FILE_PTR       f;
  _mqx_int           res;
  uint32_t           i,n;
