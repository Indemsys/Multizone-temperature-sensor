...
static const T_IO_pins_configuration BOARD_pins_conf[] =
{
  ...
  { GPIOB_BASE_PTR, PORTB_BASE_PTR,   0,    IRQ_DIS,   0,   ALT7, DSE_HI, FAST_SLEW, OD_DIS, PFE_DIS, PUPD_DIS, GP_INP,   0 }, //  PTB0  - CLKOUT
  { GPIOB_BASE_PTR, PORTB_BASE_PTR,   16,   IRQ_DIS,   0,   ALT0, DSE_HI, FAST_SLEW, OD_DIS, PFE_DIS, PUPD_DIS, GP_INP,   0 }, //  PTB16 - EXTAL32K
  { GPIOB_BASE_PTR, PORTB_BASE_PTR,   17,   IRQ_DIS,   0,   ALT0, DSE_HI, FAST_SLEW, OD_DIS, PFE_DIS, PUPD_DIS, GP_INP,   0 }, //  PTB17 - XTAL32K

  { GPIOC_BASE_PTR, PORTC_BASE_PTR,   6,    IRQ_DIS,   0,   GPIO, DSE_HI, FAST_SLEW, OD_DIS, PFE_DIS, PUPD_DIS, GP_OUT,   0 }, //  PTC6
  { GPIOC_BASE_PTR, PORTC_BASE_PTR,   7,    IRQ_DIS,   0,   GPIO, DSE_HI, FAST_SLEW, OD_DIS, PFE_DIS, PUPD_DIS, GP_OUT,   0 }, //  PTC7
...
void BSP_Init_Pins(void)
{
  ...
  for (i = 0; i < (sizeof(BOARD_pins_conf) / sizeof(BOARD_pins_conf[0])); i++)
  {
    Config_pin(BOARD_pins_conf[i]);
  }
}
...
{
   return (GPIOC->PDOR & LSHIFT(1, 7))>>7;
}
/*------------------------------------------------------------------------------
...
{
  if (state == 0) GPIOC->PCOR = LSHIFT(1, 6); // Port Clear Output Register
  else GPIOC->PSOR = LSHIFT(1, 6);            // Port Set Output Register
}
...
{
   return (GPIOC->PDOR & LSHIFT(1, 6))>>6;
}
